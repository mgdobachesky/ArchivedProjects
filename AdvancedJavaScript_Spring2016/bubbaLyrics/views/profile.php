<script type="text/javascript" src="http://ict.neit.edu/001264912/public_html/se251/bubbaLyrics/scripts/profileScript.js"></script>

<br />
<h1 style="text-align:center; font-family:arial;">This part of the site is still in development!</h1>

<!--
<div class="container">    
	<div class="row">
		<div class="col-lg-2" style="padding-left: 0px; padding-right: 0px; margin-right:25px;">
			
			<div class="container">
				 <br>
				<strong><p>Favorite Artists:</p></strong>
			  <div class="dropdown">
				<button class="btn btn-primary dropdown-toggle" type="button" id="menu1" data-toggle="dropdown">Choose One:
				<span class="caret"></span></button>
				<ul class="dropdown-menu" role="menu" aria-labelledby="menu1">
				  <li role="presentation"><a role="menuitem" tabindex="-1" href="#">Artist</a></li>
				  <li role="presentation"><a role="menuitem" tabindex="-1" href="#">Artist</a></li>
				  <li role="presentation"><a role="menuitem" tabindex="-1" href="#">Artist</a></li>
				</ul>
			  </div>
		   </div>
			
			 <div class="container">
				 <br>
				<strong><p>Favorite Songs:</p></strong>
			  <div class="dropdown">
				<button class="btn btn-primary dropdown-toggle" type="button" id="menu1" data-toggle="dropdown">Choose One:
				<span class="caret"></span></button>
				<ul class="dropdown-menu" role="menu" aria-labelledby="menu1">
				  <li role="presentation"><a role="menuitem" tabindex="-1" href="#">Song</a></li>
				  <li role="presentation"><a role="menuitem" tabindex="-1" href="#">Song</a></li>
				  <li role="presentation"><a role="menuitem" tabindex="-1" href="#">Song</a></li>
				</ul>
			  </div>
			</div>
			
			<div class="container">
				 <br>
				<strong><p>Favorite Lyrics:</p></strong>
			  <div class="dropdown">
				<button class="btn btn-primary dropdown-toggle" type="button" id="menu1" data-toggle="dropdown">Choose One:
				<span class="caret"></span></button>
				<ul class="dropdown-menu" role="menu" aria-labelledby="menu1">
				  <li role="presentation"><a role="menuitem" tabindex="-1" href="#">Lyric</a></li>
				  <li role="presentation"><a role="menuitem" tabindex="-1" href="#">Lyric</a></li>
				  <li role="presentation"><a role="menuitem" tabindex="-1" href="#">Lyric</a></li>
				</ul>
			  </div>
			</div>
			
			<button type="submit" class="btn btn-primary" style="margin-left:15px; margin-top:100px;">Update Profile</button>
		</div>
		
		<div class="col-lg-3">
			<div class="panel panel-primary">
			
			  <div class="panel-heading">Favorite Artists</div>

			<table class="table">
				<tbody>
					<tr>
						<td><a href="#">Example</a></td>
					</tr>
					<tr>
						<td><a href="#">Example</a></td>
					</tr>
					<tr>
						<td><a href="#">Example</a></td>
					</tr>
					<tr>
						<td><a href="#">Example</a></td>
					</tr>
					<tr>
						<td><a href="#">Example</a></td>
					</tr>
					<tr>
						<td><a href="#">Example</a></td>
					</tr>
					<tr>
						<td><a href="#">Example</a></td>
					</tr>
				</tbody>
			</table>
			</div>
		</div>
		
		<div class="col-lg-3">
			<div class="panel panel-primary">
			
			  <div class="panel-heading">Favorite Songs</div>

			<table class="table">
			<tbody>
					<tr>
						<td><a href="#">Example</a></td>
					</tr>
					<tr>
						<td><a href="#">Example</a></td>
					</tr>
					<tr>
						<td><a href="#">Example</a></td>
					</tr>
					<tr>
						<td><a href="#">Example</a></td>
					</tr>
					<tr>
						<td><a href="#">Example</a></td>
					</tr>
					<tr>
						<td><a href="#">Example</a></td>
					</tr>
					<tr>
						<td><a href="#">Example</a></td>
					</tr>
				</tbody>
			</table>
			</div>
		</div>
		
		<div class="col-lg-3">
			<div class="panel panel-primary">
			
				<div class="panel-heading">Favorite Lyrics</div>
			  
			  <table class="table">
			<tbody>
					<tr>
						<td><a href="#">Example</a></td>
					</tr>
					<tr>
						<td><a href="#">Example</a></td>
					</tr>
					<tr>
						<td><a href="#">Example</a></td>
					</tr>
					<tr>
						<td><a href="#">Example</a></td>
					</tr>
					<tr>
						<td><a href="#">Example</a></td>
					</tr>
					<tr>
						<td><a href="#">Example</a></td>
					</tr>
					<tr>
						<td><a href="#">Example</a></td>
					</tr>
				</tbody>
			</table>
			</div>
		</div>
		
	</div>
</div>
-->